...
func Stream(ctx context.Context, r *http.Request) (string, error) {
	...
	startBlock := r.FormValue("startblock")
	startBlockInt, _ := strconv.Atoi(startBlock)
	endBlock := r.FormValue("endblock")
	endBlockInt, _ := strconv.Atoi(endBlock)
	if startBlockInt == 0 || endBlockInt == 0 {
		return "", common.NewError("invalid_params", "Start block and end block numbers can't be zero")
	}
